...
Output ONLY the markdown content, no conversational filler.
    ...
    console.log(
      `[Compiler Job] Generated ${generatedDoc.length} characters of documentation. Committing...`
    );

    // 7. Create a blob for the new POLYDOCS.md
    const { data: docBlob } = await octokit.git.createBlob({
      owner,
      repo,
    ...
    // 8. Create a new tree with the POLYDOCS.md
    ...
    // 10. Create a new branch reference
    ...
    const { data: pullRequest } = await octokit.pulls.create({
      ...
      body: 'This PR was automatically generated by PolyDocs in response to your recent pushes. It contains an updated `POLYDOCS.md` file.',
      head: newBranchName,
      base: branch,
    });
    ...
    await supabase
      ...
      .update({
        ...
        logs: 'Documentation generated and PR created successfully.',
      })
      .eq('id', buildId);
